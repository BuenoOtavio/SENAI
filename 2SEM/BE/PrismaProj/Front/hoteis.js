...
criar.addEventListener('submit', e => {
    ...
    const data = {
        ...
        avaliacao: criar.avaliacao.value,
...
function update(btn) {
    ...
    let data = {
        ...
        valor: parseFloat(celulas[2].textContent.trim()),
        avaliacao: celulas[3].textContent.trim(),
        email: celulas[4].textContent.trim(),
        site: celulas[5].textContent.trim(),
        id_destino: parseInt(celulas[6].textContent.trim())
    };
    fetch(`${uri}/${id}`, {
        method: 'PUT',
        headers: {
